    ...
    for j=1:m
        for k=1:m
                    if tumour_in_GCs(i,j,k).state_per_time(time)==0
                        %print nothing in the figure if the GC is out of
                        %the tumour
                    ...
                    elseif  tumour_in_GCs(i,j,k).state_per_time(time)==2
                        ...
                        hold on; %the command hold on prevents from opening new figure with a new plot command
                                                           ...
                                                           % print a yellow plus for a GC that is in resting state
                    elseif tumour_in_GCs(i,j,k).state_per_time(time)==3
                        plot3(tumour_in_GCs(i,j,k).coords(1),tumour_in_GCs(i,j,k).coords(2),tumour_in_GCs(i,j,k).coords(3),'m+') % print a magenda plus for a GC that is in proliferating state
                        hold on; %the command hold on prevents from opening new figure with a new plot command
                                                           % print a magenta plus for a GC that is in necrotic state
