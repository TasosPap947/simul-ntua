        ...
        for i=1:m       % 1st space scanning, All GCs are scanned and the changes in the number of cells due to radiation are calculaeted       
            for j=1:m
                for k=1:m
                     ...
                     tumour_in_GCs(i,j,k).num_of_cells_in_S_per_time(time)= tumour_in_GCs(i,j,k).num_of_cells_in_S_per_time(time-1)*exp(-(a_S*D+b_S*D.^2));  % ADD YOUR CODE AS ABOVE, ?????? ?? S ??? ???????????
                     ...
                     tumour_in_GCs(i,j,k).num_of_cells_in_G0_per_time(time)= tumour_in_GCs(i,j,k).num_of_cells_in_G0_per_time(time-1)*exp(-(a_G0*D+b_G0*D.^2)); % ADD YOUR CODE AS ABOVE , ?????? ?? G0 ??? ??????????? 
            ...
            for j=1:m
                for k=1:m
                  ...
                  if tumour_in_GCs(i,j,k).num_of_all_cells_per_time(time)~=0
                    ...
                    %ADD YOUR CODE AS ABOVE                                                            %cells that transit from S to G2
                    if  tumour_in_GCs(i,j,k).time_spent_in_S_per_time(time-1)==T_S
                        cells_from_S_to_G2=tumour_in_GCs(i,j,k).num_of_cells_in_S_per_time(time);    
                                     
                    else
                        cells_from_S_to_G2=0;
                    end
                        
                    % ADD YOUR CODE AS ABOVE                                                            %cells that transit from G2 to M
                    if  tumour_in_GCs(i,j,k).time_spent_in_G2_per_time(time-1)==T_G2
                        cells_from_G2_to_M=tumour_in_GCs(i,j,k).num_of_cells_in_G2_per_time(time);    
                                     
                    else
                        cells_from_G2_to_M=0;
                    end
                    
                    % ADD YOUR CODE AS ABOVE                                                            %cells that transit from M to G1 or G0        %if M phase has ended, firstly the cells in M get doubled and then the transitions are made                     %USE THE PROPABILITIES FOR THE TRANSITIONS FROM PHASE TO PHASE WHERE APPLICABLE (these propabilities have been defined at the start of this file: Probs_M_to_G1 etc.)
                                                                                                                                                                                                                                                                    %the propability depends on state the GC was in the previous time step. Get for example the correct propability by typing: Probs_M_to_G1(tumour_in_GCs(i,j,k).state_per_time(time-1))
                    if  tumour_in_GCs(i,j,k).time_spent_in_M_per_time(time-1)==T_M
                        ...
                        cells_from_M_to_G1=Probs_M_to_G1(tumour_in_GCs(i,j,k).state_per_time(time-1))*tumour_in_GCs(i,j,k).num_of_cells_in_M_per_time(time);    
                        cells_from_M_to_G0=Probs_M_to_G0(tumour_in_GCs(i,j,k).state_per_time(time-1))*tumour_in_GCs(i,j,k).num_of_cells_in_M_per_time(time);             
                    ...
                    % ADD YOUR CODE AS ABOVE                                                            %cells that transit from G0 to G1 or N
                    
                    if  tumour_in_GCs(i,j,k).time_spent_in_G0_per_time(time-1)==T_G0
                        cells_from_G0_to_G1=Probs_G0_to_G1(tumour_in_GCs(i,j,k).state_per_time(time-1))*tumour_in_GCs(i,j,k).num_of_cells_in_G0_per_time(time);    
                        cells_from_G0_to_N=Probs_G0_to_N(tumour_in_GCs(i,j,k).state_per_time(time-1))*tumour_in_GCs(i,j,k).num_of_cells_in_G0_per_time(time);             
                    ...
                    if  tumour_in_GCs(i,j,k).time_spent_in_N_per_time(time-1)==T_N
                        cells_from_N_to_lysis=tumour_in_GCs(i,j,k).num_of_cells_in_N_per_time(time);    
                    ...
                    tumour_in_GCs(i,j,k).num_of_cells_in_M_per_time(time)=  tumour_in_GCs(i,j,k).num_of_cells_in_M_per_time(time) - cells_from_M_to_G1 - cells_from_M_to_G0 + cells_from_G2_to_M;  % ADD YOUR CODE AS ABOVE         %BASED ON THE CELL CYCLE SCHEME          %this code find the cells in the GC left after apoptosis and phase to phase transition (the changes to due to radiation have already been calculated in the first space scanning) 
